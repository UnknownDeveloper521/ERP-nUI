...
--   roll_material_id uuid,
--   packaging_material_id uuid,
--   material_key uuid GENERATED ALWAYS AS (COALESCE(roll_material_id, packaging_material_id)) STORED,
--   warehouse_location text,
--   available_qty numeric DEFAULT 0,
--   last_updated timestamptz DEFAULT now(),

--   CONSTRAINT raw_material_stock_choice_chk CHECK (
--     (material_type='ROLL' AND roll_material_id IS NOT NULL AND packaging_material_id IS NULL)
--  OR (material_type='PACKAGING' AND packaging_material_id IS NOT NULL AND roll_material_id IS NULL)
--   ),
--   PRIMARY KEY(material_type, material_key, warehouse_location)
-- );
